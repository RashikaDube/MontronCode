...
class PetListViewController: UIViewController {
    ...
    private func onOffButtons() {
        if let settings = viewModel.settings {
            chattingButton.isHidden = !settings.isChatEnabled
            callingButton.isHidden = !settings.isCallEnabled
        } else {
...
extension PetListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        header.headerLabel.text = "Office Hours: \(viewModel.settings?.workHours ?? "")"
        return header
    }
